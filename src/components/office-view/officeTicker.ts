...
import { AnimatedSprite, Graphics, Text, TextStyle, type Container, type Sprite } from "pixi.js";
...
export function runOfficeTickerStep(ctx: OfficeTickerContext): void {
  ...
  for (const { sprite, status, baseX, baseY, particles, agentId, cliProvider, deskG, bedG, blanketG } of ctx
    ...
    if (visual instanceof AnimatedSprite && visual.totalFrames > 1 && status !== "offline") {
  ...
  for (const clone of ctx.subCloneAnimItemsRef.current) {
    ...
    const patrolCycle = ((tick * clone.patrolSpeed + clone.phase * 32) % (patrolSpan * 2) + patrolSpan * 2) %
      (patrolSpan * 2);
    const movingRight = patrolCycle <= patrolSpan;
    const patrolX = movingRight
      ? clone.patrolMinX + patrolCycle
      : clone.patrolMaxX - (patrolCycle - patrolSpan);
    const driftX = Math.cos(wave * 1.4 + clone.phase) * SUB_CLONE_FLOAT_DRIFT;
    const driftY =
