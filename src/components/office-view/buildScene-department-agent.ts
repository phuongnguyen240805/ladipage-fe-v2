...
export function renderDeskAgentAndSubClones({
  ...
  if (isWorking && workingSubs.length > 0) {
    ...
    visibleSubs.forEach((sub, index) => {
      const lane = index % MAX_VISIBLE_SUB_CLONES_PER_AGENT;
      ...
      aura.ellipse(0, 2.0, 7.4, 2.4).fill({ color: 0x1f2937, alpha: 0.12 });
      ...
      const baseScale = (TARGET_CHAR_H / baseTexture.height) * 0.48;
      ...
      taskCard.roundRect(-8, -24, 16, 10, 2).fill({ color: 0xfff3cf, alpha: 0.92 });
      taskCard.roundRect(-8, -24, 16, 10, 2).stroke({ width: 0.8, color: themeAccent, alpha: 0.7 });
      taskCard.rect(-5, -21, 10, 1).fill({ color: 0x8b6f44, alpha: 0.75 });
      taskCard.rect(-5, -18, 7, 1).fill({ color: 0x8b6f44, alpha: 0.55 });
      ...
      subCloneAnimItemsRef.current.push({
        ...
        patrolMinX,
        patrolMaxX,
        patrolSpeed: 0.34 + (Math.abs(hashStr(`${sub.id}:speed`)) % 18) / 100,
