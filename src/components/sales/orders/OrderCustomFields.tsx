...
interface CreateFieldModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (displayName: string, fieldName: string, dataType: string) => void;
}
...
const CreateFieldModal: React.FC<CreateFieldModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const [dataType, setDataType] = useState("Dòng văn bản");
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!displayName.trim() || !fieldName.trim()) return;
    onSave(displayName.trim(), fieldName.trim(), dataType);
    setDisplayName("");
    setFieldName("");
    setFieldNameEdited(false);
    setDataType("Dòng văn bản");
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-999999 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-theme-xl w-full max-w-[440px] animate-scale-up">
        {/* Modal Header */}
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {filteredFields.length > 0 ? (
                filteredFields.map((field) => (
                  <tr key={field.id} className="hover:bg-slate-50/40 dark:hover:bg-gray-800/10 transition">
                    ...
                    <td className="py-4 px-5 text-center">
                      <button
                        onClick={() => handleDeleteField(field.id, field.displayName)}
                  ...
                  <td colSpan={5} className="py-24 text-center select-none">
                    <div className="flex flex-col items-center justify-center space-y-3">
                      ...
                      <p className="text-xs font-medium text-slate-400 dark:text-slate-500 max-w-xs text-center leading-relaxed">
                        Tạo trường đầu tiên để thêm thông tin cho đơn hàng.
                      </p>
                      <button
                        onClick={() => setIsModalOpen(true)}
