...
type SourceType = "all" | "mine" | "by-bm" | "by-id";
...
interface ConfigModalProps {
  ...
  dataOptions: PageDataOptions;
  setDataOptions: React.Dispatch<React.SetStateAction<PageDataOptions>>;
  limitApi: number;
...
const dataOptionItems: Array<{ key: keyof PageDataOptions; label: string; desc: string }> = [
    ...
    <div className="fixed inset-0 z-999999 flex animate-fade-in items-center justify-center bg-black/60 p-4 backdrop-blur-xs transition-all duration-300">
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-gray-150 bg-white shadow-2xl dark:border-gray-800 dark:bg-[#11121e]">
        <div className="flex items-center justify-between border-b border-gray-150 px-6 py-4.5 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col text-left">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white sm:text-base">Cấu hình tải dữ liệu Page</h3>
              <span className="text-[10px] text-gray-400 sm:text-xs">
                Thiết lập nguồn dữ liệu và tùy chọn trước khi tải danh sách Page.
              </span>
            </div>
          </div>
          <button
        ...
        <div className="flex max-h-[60vh] flex-col gap-5 overflow-y-auto p-6 text-left">
          <div>
            <h4 className="mb-2.5 text-xs font-bold uppercase tracking-wider text-gray-400">Phương thức tải</h4>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-4">
              {sourceOptions.map((source) => (
                <button
                  ...
                  className={`flex cursor-pointer flex-col rounded-xl border p-3.5 text-left transition-all ${
                ...
                >
                  <div className="mb-1 flex w-full items-center justify-between">
                    <span className="text-[11px] font-bold sm:text-xs">{source.title}</span>
                    {sourceType === source.id && (
                      <span className="flex h-3.5 w-3.5 items-center justify-center rounded-full bg-lime-400 text-white">
                  ...
                  <span className="text-[9px] leading-tight text-gray-400">{source.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <h4 className="mb-2.5 text-xs font-bold uppercase tracking-wider text-gray-400">Dữ liệu cần tải</h4>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              {dataOptionItems.map((option) => (
                <label
                  key={option.key}
                  className="flex cursor-pointer select-none items-start gap-2.5 rounded-xl border border-gray-150 bg-gray-50/50 p-3.5 transition hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900/40 dark:hover:bg-gray-900"
                >
                  <input
                    type="checkbox"
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-gray-150 bg-gray-50/50 p-4.5 dark:border-gray-800 dark:bg-gray-900/30">
            <div className="flex flex-col text-left">
              ...
              <span className="mt-0.5 text-[10px] text-gray-400">Số lượng Page tối đa mỗi lần gọi API phân trang</span>
            </div>
            <input
              type="number"
              value={limitApi}
