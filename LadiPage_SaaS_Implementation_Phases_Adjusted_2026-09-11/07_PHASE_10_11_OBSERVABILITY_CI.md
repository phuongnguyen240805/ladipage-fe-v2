...
## Revision 2026-09-11 — auth + UI observability/CI

### Metrics mới
- `auth_google_attempt_total{result}`
- `auth_google_provider_duration_ms`
- `auth_google_nonce_reject_total{reason}`
- `auth_refresh_total{result}`
- `auth_refresh_replay_total`
- `auth_session_created/revoked`
- `auth_legacy_password_login_total` (để biết khi nào có thể tắt MD5 path)
- `auth_cors_reject_total`
- Web Vitals theo `/signin`, `/signup`, `/p/[slug]`

Không đưa email, raw ID token, nonce, access token, refresh token vào label/log.

### CI auth contract
Fail PR nếu:
- auth store persist field tên/token-shaped credential;
- browser bundle/network còn Nest URL cho migrated feature;
- `/api/auth/*` trả access/refresh token trong JSON sau cutover;
- authenticated backend CORS quay về wildcard;
- state-changing auth route dùng GET;
- password code thêm fast hash (MD5/SHA family) cho password storage;
- protected/expensive controller thêm `@SkipThrottle()` mà không có exemption review.

### E2E/security tests bắt buộc
- Google happy path.
- wrong/expired nonce.
- replay challenge.
- malformed/expired/wrong-audience ID token.
- existing-email account link path.
- Google signup Terms behavior.
- session fixation.
- logout invalidation.
- refresh rotation race.
- permission changed giữa session.
- direct Nest endpoint reject.
- CORS exact allowlist.
- CSP Report-Only không phá Google button.
